...
struct Constants {
    ...
    static func isBlacklistedProxyDestination(_ destination: String) -> Bool {
        if proxyBlockPrivateAddresses, destination == "localhost" || destination.starts(with: "127.") || destination.starts(with: "192.") || destination.starts(with: "10.") || destination.starts(with: "172.") {
            return false
        }
